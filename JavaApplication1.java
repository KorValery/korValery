        ...
        for(int i=0; i<14; ++i){
            for(int j=0; j<18; ++j){
                if(d[i]==17){
                    ...
                    d1[i][j]=Math.asin(1.0/(Math.exp(Math.pow((Math.acos((x[j]+4.5)/17.0)/4.0), Math.tan(Math.pow(x[j]/2.0, 3.0))))));
                }else{
                    d1[i][j]=Math.sin(Math.pow(2.0*Math.sin(Math.pow(Math.PI/x[j], 3.0)), 2.0));
                }
